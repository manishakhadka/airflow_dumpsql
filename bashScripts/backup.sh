...
# SQL query to find size of the database in bytes
...
if [ $(echo "${DB_SIZE} < ${FREE_SPACE}" | bc) -eq 1 ];then
    ...
    messageOut="Backup is Successfully done in location: ${backupPath}|Required Space: ${DB_SIZE}GB, Available Space: ${FREE_SPACE}GB"
